...
router.post('/register', async (req, res) => {
  try {
    ...
    // 重複チェック
    const [exists] = await pool.query('SELECT id FROM user_table WHERE email = ? LIMIT 1', [email]);
    if (exists.length > 0) {
    ...
    // 先生ルール: createdAt TIMESTAMP NOT NULL は INSERT 時に NOW() を明示
...
router.post('/login', async (req, res) => {
  try {
    ...
    const [rows] = await pool.query('SELECT * FROM user_table WHERE email = ? LIMIT 1', [email]);
    const found = rows[0];
    if (!found) {
...
// GET /users/me (要ログイン) 動作確認用
router.get('/me', requireAuth, async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT id, email, first_name, last_name, is_admin FROM user_table WHERE id = ? LIMIT 1', [req.user.id]);
